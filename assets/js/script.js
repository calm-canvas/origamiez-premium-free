...
const Origamier = {
				...
				.hover(function () {
					if (OrigamierUtil.getViewportWidth() >= 768) {
						...
						jQuery(this)
							.find('.fa')
							.stop()
							.css('marginLeft', '-22.5px')
							.css('left', '50%')
							.transition({ opacity: 1 });
						jQuery(this)
							.find('.overlay-link')
							.stop()
							.css('marginRight', '-22.5px')
							.css('right', '50%')
							.transition({ opacity: 1 });
					}
				})
				.mouseleave(function () {
					if (OrigamierUtil.getViewportWidth() >= 768) {
						...
						jQuery(this)
							.find('.fa')
							.stop()
							.transition({ left: '100%', marginLeft: 0 });
						jQuery(this)
							.find('.overlay-link')
							.stop()
							.transition({ right: '100%', marginRight: 0 });
					}
				});
		}
	},
	...
	createMobileMenu(menu_id, mobile_menu_id, mobile_menu_class) {
		...
		jQuery(menu_id)
			.find('select')
			.first()
			.attr('id', mobile_menu_id)
			.attr('class', mobile_menu_class);
		jQuery(menu_id)
			.find('a')
			.each(function () {
				...
				jQuery(
					'<option ' +
						...
						el.text() +
						'</option>'
				).appendTo(jQuery(menu_id).find('select').first());
			});
		jQuery(menu_id)
			.find('select')
			.first()
