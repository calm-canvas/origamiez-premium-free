...
- **Dependency Injection**: 
    - Use the PHP-DI `Container` for all theme services. 
    - Avoid direct instantiation of complex classes; instead, define them in `app/Core/di-definitions.php`.
- **Hook Management**: 
    - All actions and filters should be registered via a `HookProviderInterface` class and the `HookRegistry`.
    - Group related hooks together into specialized provider classes in `app/Hooks/`.
- **Asset Development**: 
    - Source assets (SASS/JS) reside in the `assets/` directory at the project root.
    - Use **Vite** for compilation. 
    - Run `pnpm dev` during development for Hot Module Replacement (HMR).
- **Customizer Options**: 
...
1.  **Environment Setup**: 
    ...
    - Run `make up` and `./bin/init.sh` to initialize the project and restore the database snapshot.
2.  **Making Changes**: 
    - For logic changes, identify the correct `app/` sub-directory.
...
4.  **Deployment**: 
    - Always run `pnpm build` before pushing to production to ensure minified and versioned assets are generated.
    - Do not commit the `vendor/` or `node_modules/` directories.

## Related Resources
- `origamiez/app/` (Core PHP logic)
