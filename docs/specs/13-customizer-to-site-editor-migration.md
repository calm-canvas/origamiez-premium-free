...
| **Migration flag** | `update_option( 'origamiez_global_styles_migration', … )` — success payload includes **`theme_version`**, **`completed_at`**, **`wp_version`**; skip paths store a string. **The migrator runs only when** the Origamiez engine `Version` (parent when using a child theme) **equals `GlobalStylesCustomizerMigrator::MIGRATION_TARGET_VERSION`** (currently **4.4.2**). Filter: **`origamiez_global_styles_migration_target_version`**. |
...
|-----|---------------|----------|
| `origamiez_global_styles_migration` | `array( 'completed_at' => unix, 'theme_version' => '4.4.2', 'wp_version' => '6.9' )` | Set after successful one-shot migration (only attempted while engine version matches **4.4.2**). **`theme_version`** is the engine version at completion time. |
...
- Run only when **`GlobalStylesCustomizerMigrator::is_migration_target_version()`** is true (default: engine **`Version` === `4.4.2`**).
