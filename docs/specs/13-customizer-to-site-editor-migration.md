...
| **Migration flag** | `update_option( 'origamiez_global_styles_migration', … )` — success payload includes **`theme_version`**, **`completed_at`**, **`wp_version`**; skip paths store a string. **The migrator runs only when** the Origamiez engine `Version` (parent when using a child theme) **equals `GlobalStylesCustomizerMigrator::MIGRATION_TARGET_VERSION`** (currently **4.4.1**). Filter: **`origamiez_global_styles_migration_target_version`**. |
...
## 4. Migration storage (option)

| Key | Example value | Behavior |
|-----|---------------|----------|
| `origamiez_global_styles_migration` | `array( 'completed_at' => unix, 'theme_version' => '4.4.1', 'wp_version' => '6.9' )` | Set after successful one-shot migration (only attempted while engine version matches **4.4.1**). **`theme_version`** is the engine version at completion time. |
| Same key | `'skipped-incompatible-wp'` / `'skipped-no-theme-json'` | Migration did not run; bridge stays off. |

**Guards:**

- Run only for users with `edit_theme_options` (or during `after_setup_theme` with a scheduled admin notice if migration fails).
- Run only when **`GlobalStylesCustomizerMigrator::is_migration_target_version()`** is true (default: engine **`Version` === `4.4.1`**).
...
| **2 – Migration routine** | One-shot: read listed `theme_mod`s → merge into user global styles JSON; set **`origamiez_global_styles_migration`** (includes **`theme_version`**); merge policy for existing user data | **`GlobalStylesCustomizerMigrator`**: runs on **`admin_init`** (priority 30) when `edit_theme_options`, **only if** **`is_migration_target_version()`** (default: engine **`Version` === `MIGRATION_TARGET_VERSION` `4.4.1`**), then idempotent via option **`origamiez_global_styles_migration`** (also **`skipped-incompatible-wp`** / **`skipped-no-theme-json`** on old WP or no `theme.json`). Fill-missing merge for palette slugs, `fontFamilies`, `styles.elements` / `styles.blocks`, **`settings.custom.origamiez`**, and **`custom-background`** mods → **`styles.color.background`** / **`styles.background`** (image + repeat/size/position/attachment). Errors → **`origamiez_global_styles_migration_error`** + one-time **`admin_notices`**. Filters **`origamiez_global_styles_migration_enabled`**, **`origamiez_global_styles_migration_target_version`**, **`origamiez_global_styles_migration_patch`**. Requires **WordPress 5.9+** (`WP_Theme_JSON`, `wp_global_styles` CPT). Re-test: `delete_option( 'origamiez_global_styles_migration' );`. |
| **3 – Customizer UX** | Remove controls; show notice + link to `admin_url( 'site-editor.php' )` | **Done:** `SiteEditorNoticeControl` + **top-level** Customizer sections for **Colors**, **Typography**, **Google fonts**, and **Background** (one-click access; no nested panel → section). Core **`background_image`** section removed. Filter `origamiez_site_editor_admin_url`. |
| **4 – Front-end bridge** | Replace `InlineStyleGenerator` color/font reads with merged Theme JSON outputs **or** rely on `wp_enqueue_global_styles` + preset variables; update `GeneralClassProvider` for `skin` deprecation | **Done:** `ThemeJsonAppearanceBridge` (success: **`origamiez_global_styles_migration`** array with **`completed_at`** = active); merged data via `WP_Theme_JSON_Resolver::get_merged_data()` (**WP 6.2+**); colors: no legacy theme_mod inline when active (Core global styles + `_tokens.scss`); typography: inline from merged `styles` + `settings.custom.origamiez.widgetTitleTypography`; Google Fonts: `StylesheetManager` uses `get_dynamic_google_fonts_for_enqueue()` (JSON then theme_mod). **`FontManager::enqueue`** no-op (avoid duplicate handles). Body classes **`origamiez-global-styles-bridge`**, **`origamiez_custom_bg`** (uses merged JSON for background when bridge active and theme mods empty). When active, **`remove_theme_support( 'custom-background' )`** (`FrontendHooks`, `after_setup_theme` 20) avoids duplicate Core background CSS. Filters: **`origamiez_theme_json_appearance_bridge_active`**. |
