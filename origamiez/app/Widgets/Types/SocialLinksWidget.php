...
class SocialLinksWidget extends WP_Widget {
		...
		if ( ! empty( $socials ) ) :
			...
			<div class="social-link-inner clearfix">
				...
				foreach ( $socials as $social_slug => $social ) {
					$this->render_single_social_link( $social_slug, $social );
				}
				?>
			</div>
			<?php
	...
	/**
	 * Output one social link when a URL is set in theme mods.
	 *
	 * @param string $social_slug Social key.
	 * @param array  $social      Social config from origamiez_get_socials().
	 */
	private function render_single_social_link( string $social_slug, array $social ): void {
