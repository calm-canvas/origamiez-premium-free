...
class TypographySettings implements SettingsInterface {
	...
	public function register( CustomizerService $service ): void {
		$service->registerPanel( 'origamiez_typography', [
			'title' => esc_attr__( 'Typography', 'origamiez' ),
		] );

		$service->registerPanel( 'origamiez_google_fonts', [
		...
		foreach ( $font_objects as $font_slug => $font_title ) {
			$service->registerSection( "custom_{$font_slug}", [
				'panel' => 'origamiez_typography',
				'title' => $font_title
			] );

			$service->registerSetting( "{$font_slug}_is_enable", [
			...
			$service->registerSetting( "{$font_slug}_family", [
			...
			$service->registerSetting( "{$font_slug}_size", [
			...
			$service->registerSetting( "{$font_slug}_style", [
			...
			$service->registerSetting( "{$font_slug}_weight", [
			...
			$service->registerSetting( "{$font_slug}_line_height", [
		...
		if ( $number_of_google_fonts ) {
			for ( $i = 0; $i < $number_of_google_fonts; $i ++ ) {
				$service->registerSection( sprintf( 'google_font_%s', $i ), [
					'panel' => 'origamiez_google_fonts',
					'title' => esc_attr__( 'Font #:', 'origamiez' ) . ( $i + 1 )
				] );

				$service->registerSetting( sprintf( 'google_font_%s_name', $i ), [
				...
				$service->registerSetting( sprintf( 'google_font_%s_src', $i ), [
