	...
	$.fn.poptrox = function (options) {
		...
		if (settings.usePopupDefaultStyling) {
			...
			if ($caption.length > 0) {
				$popup.css(
					'padding-bottom',
					settings.popupCaptionHeight + 'px'
				);
			...
			if ($loader.length > 0) {
				$loader
					...
					.on('startSpinning', function (e) {
						var x = $(
							'<div>' + settings.popupLoaderText + '</div>'
						);

						x.css(
							'height',
							Math.floor(settings.popupHeight / 2) + 'px'
						)
							.css('overflow', 'hidden')
							.css(
								'line-height',
		...
		$popup
			...
			.on('poptrox_switch', function (e, index, ignoreLock) {
				...
				if (x.type != 'image') {
					...
					if (xwidth.slice(-1) == '%')
						xwidth =
							(parseInt(xwidth.substring(0, xwidth.length - 1)) /
								100.0) *
							$window.width();
					...
					if (xheight.slice(-1) == '%')
						xheight =
							(parseInt(
								xheight.substring(0, xheight.length - 1)
							) /
								100.0) *
							$window.height();
