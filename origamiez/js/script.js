...
let Origamier     = {
	...
	initCarouselPostRelated: function () {
		...
		if (0 < post_related.length) {
			...
			widget.find( ".fa-angle-left" ).click(
				function () {
					post_related.trigger( "owl.prev" );
				}
			);
			widget.find( ".fa-angle-right" ).click(
				function () {
	...
	initMobileMenu: function () {
		...
		jQuery( "#origamiez-mobile-wrap" ).on(
			"click",
			function () {
				jQuery( "nav#origamiez-mobile-nav" ).toggleClass( "is-active" );
			}
		);
		jQuery( "#origmiez-mobile-nav__toggle" ).on(
			"click",
			function () {
	...
	createMobileMenu: function (menu_id, mobile_menu_id, mobile_menu_class) {
		...
		jQuery( menu_id ).find( "select" ).first().change(
			function () {
